...
AutoSelector::AutoSelector(AutonomousSource* autonomousSource) {
	...
	Robot::StartLocation startingPosition =
			autonomousSource->GetStartLocation();
...
//			AddSequential(new CubeRunIntake(-1.0));			//bye bye cube!
		...
		char sideCheck =
				(startingPosition == Robot::StartLocation::right) ? 'R' : 'L'; //If starting pos. is right, sideCheck is right otherwise left
		if (doSwitch && ourSwitch == sideCheck) { // If going to switch and we can actually do the switch
			...
			if (doScale) {
				...
				if (scale == sideCheck) {
					...
					//AddSequential(new ElevatorRunToHeight(0.0, 0.0));	// TODO put in actual height or whatever
					AddSequential(new PIDDriveStraight(12));	//zoom into it
//					AddSequential(new CubeRunIntake(-1.0));			//bye bye cube
				} else {
					std::cout << "Darn, it's all the way over there"
							<< std::endl;
					...
					AddSequential(new PIDDriveStraight(100));//zooming to the middle of scale
					AddSequential(new PIDTurn(90 * -directionScale));//turning towards the scale
					//AddSequential(new ElevatorRunToHeight(0.0, 0.0));//TODO Add in correct height
					AddSequential(new PIDDriveStraight(17.5));	//zoom at scale
//					AddSequential(new CubeRunIntake(-1.0));			//bye bye cube
				}
			} else {
				std::cout << "We're normal and just gonna do the easy stuff"
						<< std::endl;
				AddSequential(new PIDTurn(90 * -directionSwitch));//turning towards the switch to drop cube
				AddSequential(new PIDDriveStraight(24)); //TODO Add in correct distance
//				AddSequential(new CubeRunIntake(-1.0));				//bye bye cube
			}
...
//				AddSequential(new CubeRunIntake(-1.0));								//bye bye cube
				...
				AddSequential(
						new UltrasonicStraightDrive(0.75, 222, trackingSide));//drive with sensors down the wall
